...
ztest_random_dnode_slots(void)
{
	...
	case 0:
		slots = 5 + ztest_random(DNODE_MAX_SLOTS - 4);
		break;
	case 1 ... 4:
...
{
	return (ztest_random_dnode_slots() * DNODE_MIN_SIZE);
}
...
ztest_bt_generate(ztest_block_tag_t *bt, objset_t *os, uint64_t object,
    uint8_t dn_slots, uint64_t offset, uint64_t gen, uint64_t txg,
    uint64_t crtxg)
{
	bt->bt_magic = BT_MAGIC;
	bt->bt_objset = dmu_objset_id(os);
	bt->bt_object = object;
	bt->bt_slots = dn_slots;
	bt->bt_offset = offset;
	bt->bt_gen = gen;
	bt->bt_txg = txg;
	bt->bt_crtxg = crtxg;
...
/*
 * Generate a unique token to fill up unused bonus buffer space.
 * Try to make it unique to the object, generation, and offset to
 * verify that data is not getting stomped on.
 */
 ...
 * verifiable pattern. Filling the whole dnode with non-zero data helps
 * ensure the all dnode traversal code properly skips the interior
 * regions of large dnodes.
 */
void
...
#define	lrz_dn_slots	lr_crtime[1]
...
ztest_replay_create(ztest_ds_t *zd, lr_create_t *lr, boolean_t byteswap)
{
	...
	bonuslen = DN_BONUS_SIZE(lr->lrz_dn_slots);

	if (lr->lrz_type == DMU_OT_ZAP_OTHER) {
		if (lr->lr_foid == 0) {
			lr->lr_foid = zap_create_slots(os,
			    lr->lrz_type, lr->lrz_bonustype,
			    bonuslen, lr->lrz_dn_slots, tx);
		} else {
			error = zap_create_claim_slots(os, lr->lr_foid,
			    lr->lrz_type, lr->lrz_bonustype,
			    bonuslen, lr->lrz_dn_slots, tx);
		}
	} else {
		if (lr->lr_foid == 0) {
			lr->lr_foid = dmu_object_alloc_slots(os,
			    lr->lrz_type, 0, lr->lrz_bonustype,
			    bonuslen, lr->lrz_dn_slots, tx);
		} else {
			error = dmu_object_claim_slots(os, lr->lr_foid,
			    lr->lrz_type, 0, lr->lrz_bonustype,
			    bonuslen, lr->lrz_dn_slots, tx);
		}
	}
	...
	ztest_bt_generate(bbt, os, lr->lr_foid, lr->lrz_dn_slots, -1ULL,
	    lr->lr_gen, txg, txg);
...
ztest_replay_setattr(ztest_ds_t *zd, lr_setattr_t *lr, boolean_t byteswap)
{
	...
	dmu_buf_t *db;
	ztest_block_tag_t *bbt;
	uint64_t txg, lrtxg, crtxg, slots;
	 ...
	 */
	ztest_bt_verify(bbt, os, lr->lr_foid, slots, -1ULL, lr->lr_mode,
	    MAX(txg, lrtxg), crtxg);
	...
	ztest_bt_generate(bbt, os, lr->lr_foid, slots, -1ULL, lr->lr_mode, txg,
	    crtxg);
...
ztest_create(ztest_ds_t *zd, ztest_od_t *od, int count)
{
	...
	for (i = 0; i < count; i++, od++) {
		...
		lr->lrz_dn_slots = od->od_crdnodeslots;
...
ztest_io(ztest_ds_t *zd, uint64_t object, uint64_t offset)
{
	...
	case ZTEST_IO_WRITE_TAG:
		ztest_bt_generate(&wbt, zd->zd_os, object, doi.doi_num_slots,
		    offset, 0, 0, 0);
...
ztest_od_init(ztest_od_t *od, uint64_t id, char *tag, uint64_t index,
    dmu_object_type_t type, uint64_t blocksize, uint8_t dn_slots, uint64_t gen)
{
	...
	od->od_crdnodeslots = dn_slots ? dn_slots : ztest_random_dnode_slots();
	od->od_crgen = gen;
...
ztest_verify_dnode_bt(ztest_ds_t *zd, uint64_t id)
{
	...
	for (obj = 0; err == 0; err = dmu_object_next(os, &obj, FALSE, 0)) {
		...
		ztest_bt_verify(bt, os, obj, doi.doi_num_slots, bt->bt_offset,
		    bt->bt_gen, bt->bt_txg, bt->bt_crtxg);
...
ztest_dsl_prop_get_set(ztest_ds_t *zd, uint64_t id)
{
	...
	VERIFY0(ztest_dsl_prop_set_uint64(zd->zd_name, ZFS_PROP_DNODESIZE,
	    ztest_random_dnodesize(), (int)ztest_random(2)));
