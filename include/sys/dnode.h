...
#define	DN_BONUS_SIZE(slots)	(((slots) << DNODE_SHIFT) - DNODE_CORE_SIZE - \
	(1 << SPA_BLKPTRSHIFT))
#define	DN_OLD_MAX_BONUSLEN	(DN_BONUS_SIZE(DNODE_MIN_SLOTS))
#define	DN_MAX_BONUSLEN		(DN_BONUS_SIZE(DNODE_MAX_SLOTS))
#define	DN_MAX_NBLKPTR	((DNODE_MIN_SIZE - DNODE_CORE_SIZE) >> SPA_BLKPTRSHIFT)
#define	DN_MAX_OBJECT	(1ULL << DN_MAX_OBJECT_SHIFT)
#define	DN_ZERO_BONUSLEN	(DN_BONUS_SIZE(DNODE_MAX_SLOTS) + 1)
...
#define	DN_BONUSLEN_TO_SLOTS(len)	((P2ROUNDUP((len) + \
	(1 << SPA_BLKPTRSHIFT) + DNODE_CORE_SIZE, DNODE_MIN_SIZE)) >> \
	DNODE_SHIFT)

#define	EPB(blkshift, typeshift)	(1 << (blkshift - typeshift))

/*
 * The number of dnode slots consumed by the object may be stored
 * in the 8 bits above the object ID.
 */
#define	DN_OID_GET_SLOTS(oid) BF64_GET((oid), DN_MAX_OBJECT_SHIFT, 8)
#define	DN_OID_SET_SLOTS(oid, x) BF64_SET((oid), DN_MAX_OBJECT_SHIFT, 8, x)
#define	DN_OID_GET_OBJ(oid) BF64_GET((oid), 0, DN_MAX_OBJECT_SHIFT)
#define	DN_OID_SET_OBJ(oid, x) BF64_SET((oid), 0, DN_MAX_OBJECT_SHIFT, x)
...
int dnode_hold_impl(struct objset *dd, uint64_t object, int flag, int count,
    void *ref, dnode_t **dnp);
