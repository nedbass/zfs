...
dbuf_read_impl(dmu_buf_impl_t *db, zio_t *zio, uint32_t *flags)
{
	...
	if (db->db_blkid == DMU_BONUS_BLKID) {
		...
		db->db.db_data = zio_buf_alloc(DN_BONUS_SIZE(dn->dn_num_slots));
		arc_space_consume(DN_BONUS_SIZE(dn->dn_num_slots),
		    ARC_SPACE_OTHER);
		if (bonuslen < DN_BONUS_SIZE(dn->dn_num_slots))
			bzero(db->db.db_data, DN_BONUS_SIZE(dn->dn_num_slots));
		if (bonuslen)
...
dbuf_fix_old_data(dmu_buf_impl_t *db, uint64_t txg)
{
	...
	if (db->db_blkid == DMU_BONUS_BLKID) {
		...
		dr->dt.dl.dr_data = zio_buf_alloc(DN_BONUS_SIZE(dn->dn_num_slots));
		arc_space_consume(DN_BONUS_SIZE(dn->dn_num_slots), ARC_SPACE_OTHER);
...
dbuf_clear(dmu_buf_impl_t *db)
{
	...
	if (db->db_state == DB_CACHED) {
		ASSERT(db->db.db_data != NULL);
		if (db->db_blkid == DMU_BONUS_BLKID) {
			uint8_t count = DB_DNODE(db)->dn_num_slots;
    ...
    dmu_buf_impl_t *parent, blkptr_t *blkptr)
{
	...
	if (blkid == DMU_BONUS_BLKID) {
		...
		db->db.db_size = DN_BONUS_SIZE(dn->dn_num_slots) -
...
dbuf_sync_leaf(dbuf_dirty_record_t *dr, dmu_tx_t *tx)
{
	...
	if (db->db_blkid == DMU_BONUS_BLKID) {
		...
		ASSERT3U(dn->dn_phys->dn_bonuslen, <=,
		    DN_BONUS_SIZE(dn->dn_phys->dn_extra_slots + 1));
