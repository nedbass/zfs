...
{
	(void) snprintf(buf, size, "%-10s %-10s %-12s %-24s %-10s %-32s\n",
...
spa_mmp_history_data(char *buf, size_t size, void *data)
{
	...
	(void) snprintf(buf, size, "%-10llu %-10llu %-12llu %-24llu %-10llu "
	    ...
	    (u_longlong_t)smh->vdev_label, smh->vdev_path);
...
spa_mmp_history_init(spa_t *spa)
{
	...
	ksp = kstat_create(name, 0, "mmp", "misc",
	    KSTAT_TYPE_RAW, 0, KSTAT_FLAG_VIRTUAL);
	ssh->kstat = ksp;
    ...
    vdev_t *vd, int label)
{
	...
	if (zfs_mmp_history == 0 && ssh->size == 0)
		return;
	...
	(void) snprintf(smh->vdev_path, sizeof (smh->vdev_path), "%s",
	...
	while (ssh->size > zfs_mmp_history) {
		ssh->size--;
		rm = list_remove_tail(&ssh->list);
...
module_param(zfs_mmp_history, int, 0644);
MODULE_PARM_DESC(zfs_mmp_history, "Historic statistics for last N MMP updates");
#endif
