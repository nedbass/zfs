...
{
	return dmu_object_alloc_slots(os, ot, blocksize, bonustype, bonuslen,
	    0, tx);
}

uint64_t
dmu_object_alloc_slots(objset_t *os, dmu_object_type_t ot, int blocksize,
    dmu_object_type_t bonustype, int bonuslen, uint8_t dn_slots, dmu_tx_t *tx)
{
	...
	if (bonuslen > 0 && dn_slots > 0) {
		ASSERT((bonustype != DMU_OT_PLAIN_FILE_CONTENTS) ||
		    (bonuslen == DN_BONUS_SIZE(dn_slots)));
	} else {
	...
	for (;;) {
		...
		if (dmu_object_next(os, &object, B_TRUE, 0) == 0)
			...
			/*
			 * Round up to the next known valid starting point for a
			 * dnode. Add 1 before rounding so we always advance.
			 */
...
{
	return (dmu_object_claim_slots(os, object, ot, blocksize, bonustype,
	    bonuslen, 0, tx));
}

int
dmu_object_claim_slots(objset_t *os, uint64_t object, dmu_object_type_t ot,
    int blocksize, dmu_object_type_t bonustype, int bonuslen, uint8_t dn_slots,
    dmu_tx_t *tx)
{
	dnode_t *dn;
	int err;
...
dmu_object_reclaim(objset_t *os, uint64_t object, dmu_object_type_t ot,
    int blocksize, dmu_object_type_t bonustype, int bonuslen, dmu_tx_t *tx)
{
	return (dmu_object_reclaim_slots(os, object, ot, blocksize, bonustype,
	    bonuslen, 0, tx));
}

int
dmu_object_reclaim_slots(objset_t *os, uint64_t object, dmu_object_type_t ot,
    int blocksize, dmu_object_type_t bonustype, int bonuslen, uint8_t slots,
    dmu_tx_t *tx)
{
	dnode_t *dn;
	int err;
	...
	dnode_reallocate(dn, ot, blocksize, bonustype, bonuslen, slots, tx);
...
dmu_object_next(objset_t *os, uint64_t *objectp, boolean_t hole, uint64_t txg)
{
	...
	if (error && !(error == EINVAL && *objectp == 0))
		...
		offset = (*objectp + doi.doi_num_slots) << DNODE_SHIFT;
