...
dnode_verify(dnode_t *dn)
{
	...
	if (dn->dn_phys->dn_type != DMU_OT_NONE || dn->dn_allocated_txg != 0) {
		...
		ASSERT3U((dn->dn_nblkptr - 1) * sizeof (blkptr_t) +
		    dn->dn_bonuslen, <=, DN_BONUS_SIZE(dn->dn_count));
		for (i = 0; i < TXG_SIZE; i++) {
...
dnode_byteswap(dnode_phys_t *dnp)
{
	...
	dnp->dn_nextra = BSWAP_8(dnp->dn_nextra);
	...
	if (dnp->dn_bonuslen != 0) {
		...
		size_t len = DN_BONUS_SIZE(dnp->dn_nextra + 1) - off;
...
dnode_setbonuslen(dnode_t *dn, int newsize, dmu_tx_t *tx)
{
	...
	ASSERT3U(newsize, <=, DN_BONUS_SIZE(dn->dn_count) -
    ...
    dmu_object_type_t bonustype, int bonuslen, dmu_tx_t *tx)
{
	...
	ASSERT3U(bonuslen, <=, DN_BONUS_SIZE(count));
	ASSERT(dn->dn_type == DMU_OT_NONE);
	...
	if (bonustype == DMU_OT_SA) /* Maximize bonus space for SA */
		...
		dn->dn_nblkptr = MIN(DN_MAX_NBLKPTR,
		    1 + ((DN_OLD_MAX_BONUSLEN - bonuslen) >>
		    SPA_BLKPTRSHIFT));
	}
    ...
    dmu_object_type_t bonustype, int bonuslen, dmu_tx_t *tx)
{
	...
	dn->dn_bonustype = bonustype;
	dn->dn_bonuslen = bonuslen;
	dn->dn_count = DN_BONUSLEN_TO_COUNT(bonuslen);
...
/*
 * Return true if the given index in the dnode block is interior to a
 * dnode already allocated in the block. That is, the index
 * is neither free nor allocated, but is consumed by a large dnode.
 *
 ...
 */
static boolean_t
dnode_is_consumed(int idx, dnode_phys_t *dn_block, dnode_children_t *dn_child)
{
	dnode_phys_t *dnp = dn_block;
	dnode_handle_t *dnh;
	dmu_object_type_t ot;
	int i;

	for (i = 0; i < idx; dnp = dn_block + i) {
		int skip;

		dnh = &dn_child->dnc_children[i];

		zrl_add(&dnh->dnh_zrlock);
		if (dnh->dnh_dnode != NULL) {
			ot = dnh->dnh_dnode->dn_type;
			skip = dnh->dnh_dnode->dn_count;
		} else {
			ot = dnp->dn_type;
			skip = dnp->dn_nextra + 1;
		}
 ...
 */
static boolean_t
dnode_is_allocated(int idx, dnode_phys_t *dn_block, dnode_children_t *dn_child)
{
	dnode_phys_t *dnp;
	dnode_handle_t *dnh;
	dmu_object_type_t ot;

	if (dnode_is_consumed(idx, dn_block, dn_child))
		return (B_FALSE);

	dnh = &dn_child->dnc_children[idx];
	dnp = dn_block + idx;
 ...
 */
static boolean_t
dnode_is_free(int start_idx, int count, dnode_phys_t *dn_block,
    dnode_children_t *dn_child)
{
	dnode_phys_t *dnp;
	dnode_handle_t *dnh;
	dmu_object_type_t ot;
	...
	if (dnode_is_consumed(start_idx, dn_block, dn_child))
		return (B_FALSE);

	for (i = 0; i < count; i++) {
		dnp = dn_block + start_idx + i;
		dnh = &dn_child->dnc_children[start_idx + i];
 ...
 * ENOSPC - hole too small to fulfill "count" request
 * EIO - i/o error.
 * succeeds even for free dnodes.
 */
int
dnode_hold_impl(objset_t *os, uint64_t object, int flag, int count,
    void *tag, dnode_t **dnp)
{
	...
	ASSERT(!(flag & DNODE_MUST_BE_ALLOCATED) || (count == 0));
	ASSERT(!(flag & DNODE_MUST_BE_FREE) || (count > 0));
	...
	if ((flag & DNODE_MUST_BE_FREE) &&
	    !dnode_is_free(idx, count, dn_block_begin, children_dnodes)) {
		dbuf_rele(db, FTAG);
		return (ENOSPC);
	} else if ((flag & DNODE_MUST_BE_ALLOCATED) &&
	    !dnode_is_allocated(idx, dn_block_begin, children_dnodes)) {
		dbuf_rele(db, FTAG);
		return (ENOENT);
	}
	...
	int lvl, uint64_t blkfill, uint64_t txg)
{
	...
	if (db != NULL && txg != 0 && (db->db_blkptr == NULL ||
		...
		while (i >= 0 && i < (blkfill * (1ULL << DNODE_SHIFT))) {
			...
			i += DNODE_MIN_SIZE;
			*offset += DNODE_MIN_SIZE;
			if (dnp->dn_type != DMU_OT_NONE) {
				i += dnp->dn_nextra * DNODE_MIN_SIZE;
				*offset += dnp->dn_nextra * DNODE_MIN_SIZE;
			}
		}
