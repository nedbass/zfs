...
void
dnode_allocate(dnode_t *dn, dmu_object_type_t ot, int blocksize,
    ...
    dmu_object_type_t bonustype, int bonuslen, dmu_tx_t *tx)
{
	...
	ASSERT((bonustype == DMU_OT_NONE && bonuslen == 0) ||
	    ...
	    (bonustype == DMU_OT_SA && bonuslen == 0));
	ASSERT(DMU_OT_IS_VALID(bonustype));
	ASSERT3U(bonuslen, <=, DN_OLD_MAX_BONUSLEN);
	...
	if (dn->dn_bonus) {
		dn->dn_bonus->db.db_size =
		    DN_OLD_MAX_BONUSLEN - (dn->dn_nblkptr-1) * sizeof (blkptr_t);
	...
	if (db != NULL && txg != 0 && (db->db_blkptr == NULL ||
		...
		while (i >= 0 && i < (blkfill * (1ULL << DNODE_SHIFT))) {
			...
			*offset += DNODE_MIN_SIZE * inc;
			if (dnp->dn_type != DMU_OT_NONE) {
				i += (dnp->dn_nextra * DNODE_MIN_SIZE) * inc;
				*offset +=
				    (dnp->dn_nextra * DNODE_MIN_SIZE) * inc;
			}
		}
