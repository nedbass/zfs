...
dnode_verify(dnode_t *dn)
{
	...
	if (dn->dn_phys->dn_type != DMU_OT_NONE || dn->dn_allocated_txg != 0) {
		...
		ASSERT3U(dn->dn_bonuslen, <=, DN_BONUS_SIZE(dn->dn_num_slots));
		ASSERT3U(dn->dn_datablksz, ==,
		    dn->dn_datablkszsec << SPA_MINBLOCKSHIFT);
		ASSERT3U(ISP2(dn->dn_datablksz), ==, dn->dn_datablkshift != 0);
		ASSERT3U((dn->dn_nblkptr - 1) * sizeof (blkptr_t) +
		    dn->dn_bonuslen, <=, DN_BONUS_SIZE(dn->dn_num_slots));
		for (i = 0; i < TXG_SIZE; i++) {
...
dnode_setbonuslen(dnode_t *dn, int newsize, dmu_tx_t *tx)
{
	...
	ASSERT3U(newsize, <=, DN_BONUS_SIZE(dn->dn_num_slots) -
    ...
    uint64_t object, dnode_handle_t *dnh)
{
	...
	dn->dn_num_slots = dnp->dn_nextra + 1;
...
{
	int i;
	int dn_slots = dmu_tx_dn_slots(tx);
	...
	ASSERT3U(bonuslen, <=, DN_BONUS_SIZE(dn_slots));
	ASSERT(dn->dn_type == DMU_OT_NONE);
	...
	if (bonustype == DMU_OT_SA) /* Maximize bonus space for SA */
		...
		dn->dn_nblkptr = MIN(DN_MAX_NBLKPTR,
		    1 + ((DN_BONUS_SIZE(dn_slots) - bonuslen) >>
		    SPA_BLKPTRSHIFT));
	}
...
{
	int nblkptr;
	int dn_slots = DN_BONUSLEN_TO_SLOTS(bonuslen);
    ...
    dnode_children_t *dn_child)
{
	...
	if (start_idx + count > DNODES_PER_BLOCK)
		return (B_FALSE);
