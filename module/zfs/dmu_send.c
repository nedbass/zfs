...
restore_object(struct restorearg *ra, objset_t *os, struct drr_object *drro)
{
	...
	dprintf("lrg_dnode=%d bonuslen=%d obj=%llu\n",
	    spa_feature_is_enabled(os->os_spa, SPA_FEATURE_LARGE_DNODE),
	    drro->drr_bonuslen, drro->drr_object);
	...
	count = DN_BONUSLEN_TO_COUNT(drro->drr_bonuslen);
	tx = dmu_tx_create_dn_count(os, count);
    ...
    struct drr_freeobjects *drrfo)
{
	...
	obj = drrfo->drr_firstobj;

	if (obj == 0)
		obj++;

	while (obj < drrfo->drr_firstobj + drrfo->drr_numobjs) {
		dmu_object_info_t doi;
		int err;
