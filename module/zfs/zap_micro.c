...
{
	return (zap_create_claim_slots(os, obj, ot, bonustype, bonuslen,
	    0, tx));
}

int
zap_create_claim_slots(objset_t *os, uint64_t obj, dmu_object_type_t ot,
    dmu_object_type_t bonustype, int bonuslen, uint8_t dn_slots, dmu_tx_t *tx)
{
	return (zap_create_claim_norm_slots(os, obj,
	    0, ot, bonustype, bonuslen, dn_slots, tx));
}
...
zap_create_claim_norm_slots(objset_t *os, uint64_t obj, int normflags,
    ...
    uint8_t dn_slots, dmu_tx_t *tx)
{
	int err;

	err = dmu_object_claim_slots(os, obj, ot, 0, bonustype, bonuslen,
	    dn_slots, tx);
	if (err != 0)
		return (err);
...
zap_create_slots(objset_t *os, dmu_object_type_t ot,
    dmu_object_type_t bonustype, int bonuslen, uint8_t dn_slots, dmu_tx_t *tx)
{
	return (zap_create_norm_slots(os, 0, ot, bonustype, bonuslen, dn_slots,
	    tx));
}
